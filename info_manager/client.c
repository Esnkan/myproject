...
int main(int argc, const char *argv[])
{
	...
	if(connect(sockfd,(const struct sockaddr *)&serveraddr,addrlen) == -1){
		perror("<< 连接服务器端 connect server failed.");
		exit(-1);
	}else{
		printf("<< 连接服务器端 connect server sucessfully.\n");
	}
...
int admin_or_user_login(int sockfd,MSG *msg)	//欢迎用户和管理员界面
{
	...
	if(strncmp(msg->recvmsg, "OK", 2) == 0)
	{
		...
		{
			printf("<< 您好，管理员！欢迎您登陆员工管理系统！\n");
			admin_menu(sockfd,msg);
		}
		else if(msg->usertype == USER)
		{
			printf("<< 您好，用户！欢迎您登陆员工管理系统！\n");
